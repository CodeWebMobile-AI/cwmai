#!/usr/bin/env python3
"""
Worker Status Monitor (Fixed Version)

Real-time monitoring tool for checking worker status, progress, and current state.
This version properly manages Redis Pub/Sub connections to avoid connection exhaustion.
"""
...
from scripts.redis_worker_coordinator import RedisWorkerCoordinator
...
# Create a specific logger for worker monitoring
...
class SingletonPubSubManager:
    """Singleton manager for Redis Pub/Sub connections."""
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(SingletonPubSubManager, cls).__new__(cls)
    ...
    async def get_redis_client(self) -> RedisClient:
        ...
        if self._redis_client is None:
            ...
            logger.info("Created singleton Redis client")
        return self._redis_client
    
    async def get_pubsub_manager(self):
        """Get or create shared Pub/Sub manager."""
        if self._pubsub_manager is None:
            from scripts.redis_integration.redis_pubsub_manager import RedisPubSubManager
            redis_client = await self.get_redis_client()
            self._pubsub_manager = RedisPubSubManager(redis_client, instance_id="monitor_singleton")
            await self._pubsub_manager.start()
            logger.info("Created singleton Pub/Sub manager")
...
class WorkerStatusMonitor:
    """Monitor and display worker status information with proper connection management."""
    ...
    def __init__(self):
        ...
        self._singleton_manager = SingletonPubSubManager()
        self._shared_redis_client = None
        logger.info("Worker Status Monitor initialized")
        
    async def initialize(self):
        """Initialize monitoring components with connection reuse."""
        logger.info("Initializing monitoring components...")
        try:
            # Get shared Redis client
            self._shared_redis_client = await self._singleton_manager.get_redis_client()
            
            # Initialize Redis components with shared client
            # Use shared Redis client for work queue to reuse connections
            self.redis_work_queue = RedisWorkQueue(redis_client=self._shared_redis_client)
            await self.redis_work_queue.initialize()
            
            # Use shared Redis client for state manager to avoid extra connections
            from scripts.redis_lockfree_state_manager import RedisLockFreeStateManager
            self.redis_state_manager = RedisLockFreeStateManager(self._shared_redis_client)
            await self.redis_state_manager.initialize()
            logger.info("Redis components initialized successfully")
            
            # Initialize worker coordinator with shared client (optional)
            try:
                self.worker_coordinator = RedisWorkerCoordinator(
                    redis_client=self._shared_redis_client,
                    worker_id="status_monitor"
                )
                # Don't initialize coordinator's own Pub/Sub - we'll use direct Redis queries
                logger.info("Worker coordinator created (without Pub/Sub)")
            except Exception as e:
                logger.warning(f"Worker coordinator not available: {e}")
                print(f"Warning: Worker coordinator not available: {e}")
    ...
    async def get_worker_status(self) -> Dict[str, Any]:
        """Get comprehensive worker status information using direct Redis queries."""
        ...
        try:
            # Get worker information from Redis directly (no Pub/Sub)
    ...
    async def close(self):
        ...
        # Don't close singleton connections here - they're managed globally
...
async def main():
    ...
    try:
        ...
        await monitor.close()
        # Close singleton connections on exit
        singleton_manager = SingletonPubSubManager()
        await singleton_manager.close()
